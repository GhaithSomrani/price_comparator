...
# Check MongoDB
echo -e "${YELLOW}[1/5] MongoDB Status ($MONGO_SERVICE):${NC}"
if systemctl is-active --quiet $MONGO_SERVICE; then
    ...
    echo -e "  ${YELLOW}  Start with: sudo systemctl start $MONGO_SERVICE${NC}"
fi
...
echo -e "  Manage services:    ${YELLOW}$SCRIPT_DIR/manage_services_updated.sh${NC}"
echo -e "  Test API:           ${YELLOW}curl http://localhost:5000/products${NC}"
echo -e "  MongoDB status:     ${YELLOW}sudo systemctl status $MONGO_SERVICE${NC}"
