...
if systemctl list-unit-files | grep -q "^mongod.service"; then
    ...
    MONGO_SERVICE="mongod"  # Default fallback
fi
...
stop_services() {
    ...
    sudo systemctl stop $MONGO_SERVICE
...
check_status() {
    echo -e "\n${YELLOW}=== MongoDB Status ($MONGO_SERVICE) ===${NC}"
    sudo systemctl status $MONGO_SERVICE --no-pager | head -n 10
...
test_mongodb() {
    ...
    echo "Service name: $MONGO_SERVICE"
    ...
    if command -v mongosh &> /dev/null; then
        if mongosh --eval "db.adminCommand('ping')" --quiet > /dev/null 2>&1; then
            ...
            PRODUCT_COUNT=$(mongosh product_comparator --eval "db.products.countDocuments()" --quiet 2>/dev/null)
            echo -e "${GREEN}✓ Database 'product_comparator' has $PRODUCT_COUNT products${NC}"
        else
            echo -e "${RED}✗ Cannot connect to MongoDB${NC}"
        fi
    else
        echo -e "${YELLOW}⚠ mongosh not installed, cannot test connection${NC}"
    fi
}
...
test_api() {
    ...
    if ! systemctl is-active --quiet price-comparator-api.service; then
        echo -e "${RED}✗ Flask API service is not running${NC}"
        return 1
    fi
    ...
    if curl -s -f http://localhost:5000/products > /dev/null 2>&1; then
        ...
        curl -s http://localhost:5000/products | head -n 3
    else
        echo -e "${RED}✗ /products endpoint failed${NC}"
    fi

    echo -e "\n\n${BLUE}2. Testing /filter endpoint:${NC}"
    if curl -s -f http://localhost:5000/filter > /dev/null 2>&1; then
        echo -e "${GREEN}✓ /filter endpoint working${NC}"
        curl -s http://localhost:5000/filter | head -n 3
    else
        echo -e "${RED}✗ /filter endpoint failed${NC}"
    fi

    echo -e "\n\n${BLUE}3. Testing /stats endpoint:${NC}"
    if curl -s -f http://localhost:5000/stats > /dev/null 2>&1; then
        echo -e "${GREEN}✓ /stats endpoint working${NC}"
        curl -s http://localhost:5000/stats | head -n 3
    else
